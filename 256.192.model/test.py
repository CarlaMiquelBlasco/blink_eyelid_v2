...
def main(args):
    ...
    for video_file in video_files:
        ...
        video_frames_folder = os.path.join('../source_videos/Frames/', video_name)
        ...
        # Preprocess original frames stored in ../source_videos/Frames:
        root_path='../source_videos/Frames'
        for folder in os.listdir(root_path):
            frames_path = os.path.join(root_path, folder, 'Original_Frames')
            preproces(args, frames_path)

            for i in range(2):
                args.eye = 'left' if i == 0 else 'right'
                print(f"Processing for {args.eye} eye")
                ...
                start_time = 0
                end_time = 0.04
                with open(os.path.join(root_path, folder, 'Predictions/predictions_'+cfg.eye+'_'+str(cfg.time_size)+'.csv'), mode='w', newline='') as file:
                ...
                print(f"Metrics computed for {folder}. Deleting both original and cropped frames in order to increase the available storage")
